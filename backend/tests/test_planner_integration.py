...
from app.agent.planning.workflow_planner import (
...
class TestConfidenceThresholdAlignment:
    def test_orchestrator_and_planner_thresholds_are_equal(self):
        ...
        """
        from app.agent.loop.orchestrator import _SLOT_MIN_CONFIDENCE
        from app.agent.planning.workflow_planner import _SLOT_HINT_MIN_CONFIDENCE
...
class TestRunSlotExtraction:
    def _call(self, query="filter amount > 1000", clarification=None, *, mock_extract=None, mock_validate=None):
        from app.agent.loop.orchestrator import _run_slot_extraction
        ...
        with patch("app.agent.loop.orchestrator.extract_slots", mock_extract or MagicMock(
            return_value=MagicMock(
                parse_error=None,
                result=_result(),
            )
        )):
            with patch("app.agent.loop.orchestrator.validate_slots", mock_validate or MagicMock(
...
class TestChatRouterSlotIntegration:
    ...
    def test_slot_clarification_returned_when_column_missing(self):
        ...
        with patch("app.agent.loop.orchestrator.extract_slots", return_value=output):
            with patch("app.agent.loop.orchestrator.validate_slots") as mock_val:
    ...
    def test_fallback_path_used_when_extraction_fails(self):
        ...
        with patch("app.agent.loop.orchestrator.extract_slots", side_effect=RuntimeError("no LLM")):
