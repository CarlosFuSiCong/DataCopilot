...
# --------------------------------------------------------------------------- #
# 3. Tool selector classification
# --------------------------------------------------------------------------- #

class TestToolSelectorClassification:
    def test_analytical_tools_in_inspect_set(self):
        from app.agent.planning.tool_selector import _INSPECT_TOOLS
        for t in [
            "profile_column", "inspect_unique_values", "summarize_numeric_column",
            "compare_groups", "correlation_summary", "distribution_summary",
            "suggest_analysis_steps",
        ]:
            assert t in _INSPECT_TOOLS, f"{t} not in _INSPECT_TOOLS"
...
class TestWorkflowValidatorIntegration:
    def test_profile_column_survives_validation(self):
        from app.agent.validation.workflow_validator import validate as validate_workflow
