"""Deterministic slot validator.

Validates a SlotExtractionResult against the active dataset schema.
All validation logic is deterministic — no LLM calls are made here.

Validation steps (in order):
1. Unsupported intent → blocked.
2. Required slots missing for the intent → clarification.
3. Column not in schema:
   a. Fuzzy-match candidates exist → ambiguous clarification with suggestions.
   b. No candidates → missing column clarification.
4. Value type incompatible with column dtype → invalid type clarification.
...
# dtype prefixes that indicate a numeric column
_NUMERIC_DTYPES = ("int", "float", "uint", "Int", "Float")

# Maximum fuzzy-match candidates to surface in a clarification question
_MAX_CANDIDATES = 3
...
class SlotValidationOutcome(BaseModel):
    """Result of deterministic slot validation against a dataset schema.

    Fields:
        status:                  Classification of the validation result.
        is_valid:                True only when status == "valid".
        needs_clarification:     True when the user should be asked a question.
        blocked:                 True for unsupported intents.
        clarification_question:  Human-readable question to ask the user.
        candidate_columns:       Fuzzy-match suggestions for ambiguous column.
        details:                 Internal diagnostic note (not shown to users).
    """
...
def _find_column(name: str, schema_columns: list[str]) -> str | None:
    """Return the exact column if it exists (case-insensitive)."""
...
def _fuzzy_candidates(name: str, schema_columns: list[str]) -> list[str]:
    """Return close-match column names using difflib."""
...
def _resolve_candidates(fuzzy_lower: list[str], schema_columns: list[str]) -> list[str]:
    """Map lowercase fuzzy matches back to original-case column names."""
...
def _is_numeric_column(dtype: str) -> bool:
    return any(dtype.startswith(p) for p in _NUMERIC_DTYPES)


def _value_compatible(value: str, dtype: str) -> bool:
    """Return True when value is compatible with the column dtype."""
    if _is_numeric_column(dtype):
        try:
            ...
            return False
    return True  # string / datetime columns accept any string value
...
def validate_slots(
    ...
    """Validate extracted slots against a dataset schema.

    Returns a SlotValidationOutcome describing whether slots are valid,
    need clarification, or are blocked.
    """
    ...
    # 3. Column validation (only when a column slot is present)
    column = slots.column
    if column:
        exact = _find_column(column, schema_columns)
        if exact is None:
            candidates_lower = _fuzzy_candidates(column, schema_columns)
            candidates = _resolve_candidates(candidates_lower, schema_columns)
            if candidates:
                return _ambiguous_column(column, candidates)
            return _missing_column(column, schema_columns)

        # 4. Value type validation (only for filter intent with numeric column)
